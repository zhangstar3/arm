...
class MoveItDemo:
    def __init__(self):
        ...
        manipulator = moveit_commander.MoveGroupCommander('arm')
        
        # Set a small tolerance on joint angles
        manipulator.set_goal_joint_tolerance(0.005)
       
        #right_gripper.set_goal_joint_tolerance(0.001)
        
        #joint_positions1 = [2.5, 1.0, -2.6, 0.0,3.14]
        #joint_positions2 = [-2.5, 1.0, 2.6, -1.0,-3.14]
        
        joint_positions1 = [1.5, 0, 0,0,0]
        joint_positions2 = [-1.5,0, 0,0,0]

        manipulator.set_joint_value_target(joint_positions1)
        traj = manipulator.plan()
        manipulator.execute(traj)
        #manipulator.go()
        rospy.sleep(0.3)
     
        manipulator.set_joint_value_target(joint_positions2)
        manipulator.go()
        rospy.sleep(0.3)
